...
type=$(echo $1 | cut -c1)
slot=$(echo $0 | cut -d'_' -f2 | cut -d'.' -f1)

if [ $type == "C" ]; then
    rewrite=$(echo $1 | cut -dC -f2 | sed "s/D/)(/g" | sed "s/*/,/g")
    sed -i "s/^$slot=.*$/$slot=|t($rewrite)/g" $RPTCONF
    ...
    sed -i "s/^$slot=.*$/$slot=|m$mychar/g" $RPTCONF
    asterisk -rx "rpt localplay $MYNODE rpt/write_c_t"
    sleep 2
    asterisk -rx "module reload"
    exit
    ...
    cat $VOCAB | grep $myword | cut -d' ' -f2 >$SOUNDS/$slot.ulaw
    sed -i "s/^$slot=.*$/$slot=$slot/g" $RPTCONF
    asterisk -rx "rpt localplay $MYNODE rpt/write_c_t"
    sleep 2
    asterisk -rx "module reload"
    exit
