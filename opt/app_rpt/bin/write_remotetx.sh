...
if [ $type == "C" ]; then
    ...
    sed -i "s/^remotetx=.*$/remotetx=|t($rewrite)/g" $RPTCONF
    ...
    sed -i "s/^remotetx=.*$/remotetx=|m$mychar/g" $RPTCONF
    asterisk -rx "rpt localplay $MYNODE rpt/write_c_t"
    sleep 2
    asterisk -rx "module reload"
    exit
    ...
    cat $VOCAB | grep $myword | cut -d' ' -f2 >$SOUNDS/remotetx.ulaw
    sed -i "s/^remotetx=.*$/remotetx=remotetx/g" $RPTCONF
    asterisk -rx "rpt localplay $MYNODE rpt/write_c_t"
    sleep 2
    asterisk -rx "module reload"
    exit
