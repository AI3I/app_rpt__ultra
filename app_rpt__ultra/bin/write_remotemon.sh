...
if [ $type == "C" ]; then
    ...
    sed -i "s/^remotemon=.*$/remotemon=|t($rewrite)/g" $RPTCONF
    ...
    sed -i "s/^remotemon=.*$/remotemon=|m$mychar/g" $RPTCONF
    asterisk -rx "rpt localplay $MYNODE rpt/write_c_t"
    sleep 2
    asterisk -rx "module reload"
    exit
    ...
    cat $VOCAB | grep $myword | cut -d' ' -f2 >$SOUNDS/remotemon.ulaw
    sed -i "s/^remotemon=.*$/remotemon=remotemon/g" $RPTCONF
    asterisk -rx "rpt localplay $MYNODE rpt/write_c_t"
    sleep 2
    asterisk -rx "module reload"
    exit
